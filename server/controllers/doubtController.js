...
/* -------------------------------
   3️⃣ Get all doubts (Admin / Users)
-------------------------------- */
export const getAllDoubts = async (req, res) => {
  try {
    const query = req.user?.role === "ADMIN" ? {} : { status: { $ne: "HIDDEN" } };
...
export const getMyDoubts = async (req, res) => {
  try {
    ...
    const doubts = await Doubt.find({ user: req.user._id })
      .populate("user", "_id name email")            // ✅ ADD THIS LINE
      .populate("replies.user", "_id name email")    // ✅ keep replies populated
      .sort({ createdAt: -1 });
...
/* -------------------------------
   7️⃣ Approve a reply (+5 reputation)
-------------------------------- */
/* -------------------------------
   ✅ Approve or Unapprove a reply (+5 / -5 reputation)
-------------------------------- */
...
export const approveReply = async (req, res) => {
  try {
    const { doubtId, replyId } = req.params;
    const doubt = await Doubt.findById(doubtId).populate("replies.user", "name email");

    if (!doubt) return res.status(404).json({ message: "Doubt not found" });

    // ✅ Only asker can approve/unapprove replies
    if (doubt.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "Not authorized to modify replies" });
    }
    ...
    // ✅ Update doubt status
    const anyApproved = doubt.replies.some((r) => r.approved);
    doubt.status = anyApproved ? "RESOLVED" : "PENDING";

    // ✅ Dynamic reputation update (+5 / -5)
    await updateReputation(reply.user._id, reply.approved ? 5 : -5);

    await doubt.save();

    res.json({
      message: reply.approved
        ? "✅ Reply approved (+5 reputation)"
        : "❎ Reply unapproved (−5 reputation)",
      doubt,
    });
  } catch (error) {
    console.error("Approve/unapprove reply error:", error);
    res.status(500).json({ message: "Error toggling reply approval" });
  }
};
...
/* -------------------------------
   8️⃣ Dashboard stats (Dashboard Page)
-------------------------------- */
export const getDashboardStats = async (req, res) => {
  try {
    const total = await Doubt.countDocuments();
